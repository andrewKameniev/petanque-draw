    ...
    <div>
        ...
        <div class="column play-off-stage-wrapper" v-if="playOffBracket">
            ...
            <Fragment v-else>
                ...
                <div class="game-row" v-for="(game, ind) in playOffBracket[currentPlayOffBracketIndex].teams" :key="ind">
...
export default {
    ...
    methods: {
        saveResults() {
            ...
            if(this.playOffBracket[this.currentPlayOffBracketIndex].teams.some(game => resultsError(game))){
                this.scoreError = true;
                return false
            }
            if(this.playOffBracket[this.currentPlayOffBracketIndex].teamsCount === 2){ //final
                this.playOffStageCurrent = 0
                this.$emit('finishTournament');
            } else {
                this.playOffBracket[this.currentPlayOffBracketIndex].teams.forEach((game, index) => {
                    if(index % 2 === 0){
                        this.playOffBracket[this.currentPlayOffBracketIndex + 1].teams[index / 2].team_1 = game.team_1_score > game.team_2_score ? game.team_1 : game.team_2
                    } else {
                        this.playOffBracket[this.currentPlayOffBracketIndex + 1].teams[(index - 1) / 2].team_2 = game.team_1_score > game.team_2_score ? game.team_1 : game.team_2
                    }
                })
    ...
    computed: {
        ...
        currentPlayOffBracketIndex(){
            if(this.playOffBracket){
                return this.playOffBracket.findIndex(item => item.stageLabel === this.playOffStageCurrent)
            } else {
                return '0'
            }
        }
