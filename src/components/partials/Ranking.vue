<template>
  <div>
    ...
    <div v-if="tournament.tournamentIsFinished && !isSwissOnly && !isBarrageOnly" class="mb-5">
      ...
      <div v-if="!isForProtocol && podiumTeams.length >= 3" class="podium">
        <div class="podium__desktop">
          <div class="podium__item podium__item--silver">
            <div class="podium__avatar">2</div>
            <div class="podium__name">{{ podiumTeams[1].title }}</div>
            <div v-if="podiumTeams[1].players" class="podium__players">
              <span v-for="(p, i) in podiumTeams[1].players" :key="i"
                >{{ p.name }} {{ p.surname || '' }}<span v-if="i < podiumTeams[1].players.length - 1">, </span></span
              >
            </div>
            <div class="podium__bar podium__bar--silver"></div>
          </div>
          <div class="podium__item podium__item--gold">
            <div class="podium__avatar">1</div>
            <div class="podium__name">{{ podiumTeams[0].title }}</div>
            <div v-if="podiumTeams[0].players" class="podium__players">
              <span v-for="(p, i) in podiumTeams[0].players" :key="i"
                >{{ p.name }} {{ p.surname || '' }}<span v-if="i < podiumTeams[0].players.length - 1">, </span></span
              >
            </div>
            <div class="podium__bar podium__bar--gold"></div>
          </div>
          <div class="podium__item podium__item--bronze">
            <div class="podium__avatar">3</div>
            <div class="podium__name">{{ podiumTeams[2].title }}</div>
            <div v-if="podiumTeams[2].players" class="podium__players">
              <span v-for="(p, i) in podiumTeams[2].players" :key="i"
                >{{ p.name }} {{ p.surname || '' }}<span v-if="i < podiumTeams[2].players.length - 1">, </span></span
              >
            </div>
            <div class="podium__bar podium__bar--bronze"></div>
          </div>
        </div>
        <div class="podium__mobile">
          <div class="podium__mobile-card podium__mobile-card--silver">
            <span class="podium__mobile-card-place">2</span>
            <div class="podium__mobile-card-info">
              <span class="podium__mobile-card-name">{{ podiumTeams[1].title }}</span>
              <span
                v-if="podiumTeams[1].players && podiumTeams[1].players.length && !isTetATet"
                class="podium__mobile-card-players"
              >
                <span v-for="(p, i) in podiumTeams[1].players" :key="i"
                  >{{ p.name }} {{ p.surname || '' }}<span v-if="i < podiumTeams[1].players.length - 1">, </span></span
                >
              ...
              <span
                v-else-if="isTetATet && resolvedClubMap[podiumTeams[1].title]"
                class="podium__mobile-card-players"
                >{{ resolvedClubMap[podiumTeams[1].title] }}</span
              >
            </div>
          </div>
          <div class="podium__mobile-card podium__mobile-card--bronze">
            <span class="podium__mobile-card-place">3</span>
            <div class="podium__mobile-card-info">
              <span class="podium__mobile-card-name">{{ podiumTeams[2].title }}</span>
              <span
                v-if="podiumTeams[2].players && podiumTeams[2].players.length && !isTetATet"
                class="podium__mobile-card-players"
              >
                <span v-for="(p, i) in podiumTeams[2].players" :key="i"
                  >{{ p.name }} {{ p.surname || '' }}<span v-if="i < podiumTeams[2].players.length - 1">, </span></span
              ...
              <span
                v-else-if="isTetATet && resolvedClubMap[podiumTeams[2].title]"
                class="podium__mobile-card-players"
                >{{ resolvedClubMap[podiumTeams[2].title] }}</span
              >
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    podiumTeams() {
      const ranking = this.showInSaved ? this.tournament.ranking : this.tournamentRanking;
      if (!ranking?.length || ranking.length < 3) return [];
      return ranking.slice(0, 3).map((team) => ({
        ...team,
        players: this.showInSaved ? team.players : this.getTeamPlayers(team.title) || null,
      }));
    },
...
.podium__item--gold .podium__avatar {
  background: var(--color-badge-gold-bg);
  border: 3px solid var(--color-badge-gold-border);
  color: var(--color-badge-gold-border);
}

.podium__item--silver .podium__avatar {
  background: var(--color-badge-silver-bg);
  border: 3px solid var(--color-badge-silver-border);
  color: var(--color-badge-silver-border);
}

.podium__item--bronze .podium__avatar {
  background: var(--color-badge-bronze-bg);
  border: 3px solid var(--color-badge-bronze-border);
  color: var(--color-badge-bronze-border);
}
...
.podium__bar--gold {
  height: 80px;
  background: var(--color-badge-gold-bg);
  border: 2px solid var(--color-badge-gold-border);
  border-bottom: none;
}
...
.podium__bar--silver {
  height: 56px;
  background: var(--color-badge-silver-bg);
  border: 2px solid var(--color-badge-silver-border);
  border-bottom: none;
}
...
.podium__bar--bronze {
  height: 40px;
  background: var(--color-badge-bronze-bg);
  border: 2px solid var(--color-badge-bronze-border);
  border-bottom: none;
}
...
.podium__mobile-card--silver {
  border-left: 4px solid var(--color-badge-silver-border);
  background: var(--color-badge-silver-bg);
}

.podium__mobile-card--bronze {
  border-left: 4px solid var(--color-badge-bronze-border);
  background: var(--color-badge-bronze-bg);
}

.podium__mobile-card-place {
  font-size: 1.25rem;
  font-weight: 800;
  min-width: 1.5rem;
}

.podium__mobile-card--silver .podium__mobile-card-place {
  color: var(--color-badge-silver-border);
}

.podium__mobile-card--bronze .podium__mobile-card-place {
  color: var(--color-badge-bronze-border);
}

.podium__mobile-card-info {
  min-width: 0;
  flex: 1;
}

.podium__mobile-card-name {
  display: block;
  font-size: 0.95rem;
  font-weight: 700;
  color: var(--color-text);
