    ...
    <div class="content tabs-content">
        <div v-if="tournament.games?.length || hasPlayOffResults">
            <div v-if="(isForProtocol && !onlyPlayOff) || !isForProtocol">
                <div
                    v-if="
                        !isForProtocol && (tournament.games?.length || tournament.cadrage?.length || hasPlayOffResults)
                    "
                    class="round-tabs-row mb-4"
                >
                    <div class="round-tabs">
                        <button
                            v-for="(round, index) in allDisplayRounds"
                            :key="index"
                        ...
                        <button
                            v-if="tournament.cadrage?.length"
                            class="button is-small mr-1 mb-1"
                        ...
                        <button
                            v-if="hasPlayOffResults && !onlyQualifying"
                            class="button is-small mr-1 mb-1"
                ...
                <div
                    v-if="selectedRound !== 'playoff' && selectedRound !== 'cadrage' && cardView"
                        ...
                        <template v-if="selectedRound === -1 || selectedRound === index">
                            ...
                            <div
                                ...
                                :class="{
                                    ...
                                    'match-item--in-progress':
                                        !tournament.tournamentIsFinished && game.status === 'in_progress',
                                    'match-item--upcoming':
                            ...
                            >
                                ...
                                <span class="match-vs">
                                    <template v-if="game.status === 'finished' || game.status === 'in_progress'">
                                        <span class="match-score"
                                            >{{ game.team_1_score ?? 0 }} : {{ game.team_2_score ?? 0 }}</span
                                        >
                                    </template>
                                    <template v-else>
                                ...
                                <div v-if="game.score_history && game.score_history.length" class="score-history">
                                    <span
                                        v-for="(entry, ei) in game.score_history"
                                        :key="ei"
                                        class="score-history__chip"
                                    >
                                ...
                                <tr v-for="(game, i) in round" :key="i">
                                    ...
                                    <td class="has-text-centered is-narrow">
                                        <strong v-if="game.team_1_score != null"
                                            >{{ game.team_1_score }} : {{ game.team_2_score }}</strong
                                        >
                ...
                <div class="table-container" v-else-if="selectedRound !== 'playoff'">
                    <table
                        class="table"
                        :class="{
                            'is-striped': !isForProtocol,
                            'is-bordered': isForProtocol,
                            'is-fullwidth': !isForProtocol,
                        }"
                    >
                        <tbody>
                            <template v-for="(round, index) in allSortedRounds" :key="index">
                                <template v-if="selectedRound === -1 || selectedRound === index">
                                    <tr
                                        v-for="(game, i) in round"
                                        :key="i"
                                        :class="{ 'search-highlight': isGameHighlighted(game) }"
                                    >
                                        <td v-if="selectedRound === -1" class="is-narrow round-group-cell">
                                            <small class="round-badge">{{ getRoundLabel(index) }}</small>
                                            <small v-if="hasGroupsColumn && game.group != null" class="group-label">{{
                                                groupsNames[game.group]
                                            }}</small>
                                        </td>
                                        <td v-if="hasGroupsColumn" class="group-cell-desktop">
                                            <template v-if="game.group != null"
                                                ><small>{{ $t('common.group') }}</small>
                                                {{ groupsNames[game.group] }}</template
                                            >
                                        </td>
                                        <td
                                        ...
                                        <td :class="{ 'has-text-weight-bold': game.team_2_score > game.team_1_score }">
                                            {{ game.team_2 }}
                                        </td>
                ...
                <div
                    v-if="
                        hasPlayOffResults &&
                        !onlyQualifying &&
                        (isForProtocol || selectedRound === -1 || selectedRound === 'playoff')
                    "
                    :class="{ 'pdf-page-break': isForProtocol, 'playoff-section': true }"
                >
                    ...
                    <template v-if="cardView && !isForProtocol">
                        <template v-for="(stage, index) in tournament.playOffBracket.stages" :key="'po' + index">
                            <template
                                v-if="
                                    stageHasContent(stage) && stage.stageLabel !== 'cadrage' && stage.stageLabel !== 1
                                "
                            >
                                ...
                                <div class="results-card-list">
                                    ...
                                    >
                                        ...
                                        <span class="match-vs">
                                            <span class="match-score"
                                                >{{ game.team_1_score ?? '--' }} : {{ game.team_2_score ?? '--' }}</span
                                            >
                                        </span>
                                        <span
                                            class="match-team"
                        ...
                        <template
                            v-if="
                                tournament.playOffBracket.thirdPlace &&
                                Object.keys(tournament.playOffBracket.thirdPlace).length !== 0
                            "
                        >
                        ...
                        <template v-for="(stage, index) in tournament.playOffBracket.stages" :key="'po' + index">
                            ...
                            >
                                ...
                                <div v-for="(game, i) in stage.teams" :key="'s' + index + 'g' + i" class="playoff-game">
                                    <span
                                        class="playoff-team playoff-team-right"
                                        :class="{
                                            'has-text-weight-bold':
                                                !isForProtocol && Number(game.team_1_score) > Number(game.team_2_score),
                                        }"
                                        >{{ isForProtocol ? teamTitles[game.team_1] : game.team_1 }}</span
                                    >
                                    <span class="playoff-score"
                                        ><strong
                                            >{{ game.team_1_score != null ? game.team_1_score : '--' }} :
                                            {{ game.team_2_score != null ? game.team_2_score : '--' }}</strong
                                        ></span
                                    >
                                    <span
                                        class="playoff-team"
                                        :class="{
                                            'has-text-weight-bold':
                                                !isForProtocol && Number(game.team_2_score) > Number(game.team_1_score),
                                        }"
                        ...
                        >
                            ...
                            <div class="playoff-game">
                                <span
                                    ...
                                    >{{
                                        isForProtocol
                                            ? teamTitles[tournament.playOffBracket.thirdPlace.team_1]
                                            : tournament.playOffBracket.thirdPlace.team_1
                                    }}</span
                                >
                                <span class="playoff-score"
                                    ><strong
                                        >{{
                                            tournament.playOffBracket.thirdPlace.team_1_score != null
                                                ? tournament.playOffBracket.thirdPlace.team_1_score
                                                : '--'
                                        }}
                                        :
                                        {{
                                            tournament.playOffBracket.thirdPlace.team_2_score != null
                                                ? tournament.playOffBracket.thirdPlace.team_2_score
                                                : '--'
                                        }}</strong
                                    ></span
                                >
                                <span
                                    ...
                                    >{{
                                        isForProtocol
                                            ? teamTitles[tournament.playOffBracket.thirdPlace.team_2]
            ...
            <div
                v-if="
                    tournament.cadrage &&
                    (isForProtocol || selectedRound === -1 || selectedRound === 'cadrage') &&
                    selectedRound !== 'playoff'
                "
            >
                <div class="mb-5">
                    <h3 class="has-text-centered mt-4" v-if="selectedRound === -1 || isForProtocol">
                        {{ $t('games.cadrage') }}
                    </h3>
                    <div v-if="cardView && !isForProtocol" class="results-card-list">
                        ...
                        >
                            <span
                                ...
                                :class="{
                                    'match-team--winner':
                                        game.status === 'finished' && game.team_1_score > game.team_2_score,
                                }"
                                >{{ game.team_1 }}</span
                            >
                            <span class="match-vs">
                            ...
                            <span
                                ...
                                :class="{
                                    'match-team--winner':
                                        game.status === 'finished' && game.team_2_score > game.team_1_score,
                                }"
                                >{{ game.team_2 }}</span
                            >
                            <div v-if="game.score_history && game.score_history.length" class="score-history">
                            ...
                            <tbody>
                                ...
                                >
                                    <td
                                        ...
                                        :class="{
                                            'has-text-weight-bold':
                                                !isForProtocol && game.team_1_score > game.team_2_score,
                                        }"
                                    >
                                    ...
                                    <td
                                        :class="{
                                            'has-text-weight-bold':
                                                !isForProtocol && game.team_2_score > game.team_1_score,
                                        }"
                                    >
                                        {{ isForProtocol ? teamTitles[game.team_2] : game.team_2 }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
import {
    ...
    hasPlayOffResults as checkPlayOffResults,
    sortGamesByGroup,
    ...
    methods: {
        ...
        saveEditedResult({ score1, score2 }) {
            ...
            for (let r = 0; r < this.tournament.games.length; r++) {
                const idx = this.tournament.games[r].findIndex(
                    (g) => g.team_1 === game.team_1 && g.team_2 === game.team_2,
                );
                if (idx !== -1) {
                    roundIndex = r;
                    gameIndex = idx;
