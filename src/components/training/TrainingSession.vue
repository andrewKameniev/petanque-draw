    ...
    <div class="tsession">
        ...
        <div v-if="isEditing" class="tsession__edit">
            ...
            <div class="tsession__edit-actions">
                <button class="button btn-primary-outline btn-sm" @click="isEditing = false">{{ $t('common.done') }}</button>
        ...
        <div v-if="session.config.exercises.length > 1" class="tir-pview__tabs">
            <button v-for="exIdx in session.config.exercises" :key="exIdx"
                    class="tir-pview__tab"
                    :class="{'tir-pview__tab--active': activeExercise === exIdx, 'tir-pview__tab--complete': isExerciseComplete(exIdx)}"
        ...
        <div class="tir-pview__atelier-card">
            <div class="tir-pview__atelier-card-header">
                ...
                <span class="tir-pview__atelier-card-score">{{ getExerciseScore(activeExercise) }}/{{ exerciseMaxScore }}</span>
            </div>
            ...
            <div v-for="distance in session.config.distances" :key="distance" class="tsession__distance-block">
                ...
                <div class="tsession__circles-table">
                    <div v-for="opt in resultOptions" :key="opt.key" class="tsession__score-row">
                        <span class="tsession__score-row-label" :class="`tsession__score-row-label--${opt.key}`">{{ $t(`tir.${opt.key}`) }}</span>
                        <div class="tsession__score-row-circles">
                            <span v-for="attemptNum in session.config.attempts" :key="attemptNum"
                                  class="tir-pview__circle"
                                  :class="[`tir-pview__circle--${opt.key}`, {'tir-pview__circle--active': getAttemptScore(activeExercise, distance, attemptNum) === opt.key}]"
        ...
        <div class="tsession__nav">
            <button v-if="session.config.exercises.length > 1 && activeExercise > session.config.exercises[0]"
                    class="tir-pview__nav-btn" @click="prevExercise">
                <ChevronLeft :size="16"/>
                {{ $t('tir.prevAtelier') }}
            </button>
            <div class="tsession__nav-spacer"></div>
            <button v-if="session.config.exercises.length > 1 && activeExercise < session.config.exercises[session.config.exercises.length - 1]"
                    class="tir-pview__nav-btn" @click="nextExercise">
                {{ $t('tir.nextAtelier') }}
                <ChevronRight :size="16"/>
            </button>
            <button v-if="session.status !== 'completed'"
                    class="tir-pview__nav-btn tir-pview__nav-btn--primary" @click="completeSession">
                <CheckIcon :size="16"/>
                {{ $t('training.complete') }}
            </button>
        </div>
...
<script>
import {ChevronLeft, ChevronRight, Pencil, Check as CheckIcon, RotateCcw} from "lucide-vue-next";
...
export default {
    ...
    components: {ChevronLeft, ChevronRight, Pencil, CheckIcon, RotateCcw},
    props: {
...
.tsession__score-row-label--carreau { color: var(--tir-carreau); }
.tsession__score-row-label--reussi { color: var(--tir-reussi); }
.tsession__score-row-label--touche { color: var(--tir-touche); }
.tsession__score-row-label--manque { color: var(--tir-manque); }
