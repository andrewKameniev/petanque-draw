    ...
    return {
      points: {
        volume: playersStat.reduce(
          (acc, item) => acc + getFrenchStat(item.points.volume, item.serie.filter((item) => item.type === 'p').length),
          0,
        ),
        intensity: playersStat.reduce(
          (acc, item) =>
            acc + getFrenchStat(item.points.intensity, item.serie.filter((item) => item.type === 'p').length),
          0,
        ),
      },
      tirs: {
        volume: playersStat.reduce(
          (acc, item) => acc + getFrenchStat(item.tirs.volume, item.serie.filter((item) => item.type === 't').length),
          0,
        ),
        intensity: playersStat.reduce(
          (acc, item) =>
