...
describe('updateScoreHistory', () => {
    ...
    it('each cochonette for same team creates a new chip', () => {
        ...
        game.team_1_score = 13; game.team_2_score = 0;
        updateScoreHistory(game);

        game.team_1_score = 13; game.team_2_score = 1;
        updateScoreHistory(game);

        game.team_1_score = 13; game.team_2_score = 2;
        updateScoreHistory(game);

        game.team_1_score = 13; game.team_2_score = 4;
        updateScoreHistory(game);

        game.team_1_score = 13; game.team_2_score = 8;
        updateScoreHistory(game);

        game.team_1_score = 13; game.team_2_score = 13;
        updateScoreHistory(game);
    ...
    it('alternating teams each create new chips', () => {
        ...
        game.team_1_score = 2; game.team_2_score = 0;
        updateScoreHistory(game);

        game.team_1_score = 2; game.team_2_score = 1;
        updateScoreHistory(game);

        game.team_1_score = 3; game.team_2_score = 1;
        updateScoreHistory(game);

        game.team_1_score = 3; game.team_2_score = 4;
        updateScoreHistory(game);
    ...
    it('ignores if score goes down (user mid-edit)', () => {
        ...
        game.team_1_score = 5; game.team_2_score = 1;
        updateScoreHistory(game);
    ...
    it('handles both scores going up as new chip', () => {
        ...
        game.team_1_score = 5; game.team_2_score = 2;
        updateScoreHistory(game);
        expect(game.score_history).toEqual([{s1: 3, s2: 0}, {s1: 5, s2: 2}]);
    });
    ...
    it('full game scenario: 2-0 2-1 3-1 3-10 3-11 12-11 12-13', () => {
        ...
        const steps = [[2,0],[2,1],[3,1],[3,10],[3,11],[12,11],[12,13]];
    ...
    it('one team scores multiple consecutive cochonettes (team 1)', () => {
        ...
        const steps = [[1,0],[2,0],[5,0],[8,0],[13,0]];
        for (const [a, b] of steps) {
            game.team_1_score = a;
            game.team_2_score = b;
            updateScoreHistory(game);
    ...
    it('ignores decrease on team_1 (user clearing to retype)', () => {
        const game = makeGame(5, 3, [{s1: 2, s2: 0}, {s1: 2, s2: 3}, {s1: 5, s2: 3}]);

        game.team_1_score = 1; game.team_2_score = 3;
        updateScoreHistory(game);
        expect(game.score_history.length).toBe(3);

        game.team_1_score = 0; game.team_2_score = 3;
        updateScoreHistory(game);
        expect(game.score_history.length).toBe(3);
    });

    it('ignores decrease on team_2 (user clearing to retype)', () => {
        const game = makeGame(3, 9, [{s1: 3, s2: 0}, {s1: 3, s2: 9}]);
    ...
    it('score going up after a decrease is a new chip', () => {
        ...
        game.team_1_score = 5; game.team_2_score = 11;
        updateScoreHistory(game);
        expect(game.score_history).toEqual([{s1: 5, s2: 9}, {s1: 5, s2: 11}]);
    });
    ...
    it('tight game with many lead changes', () => {
        ...
        const steps = [[1,0],[1,2],[3,2],[3,5],[6,5],[6,7],[8,7],[8,10],[11,10],[11,13]];
        for (const [a, b] of steps) {
            game.team_1_score = a;
            game.team_2_score = b;
            updateScoreHistory(game);
    ...
    it('does not add chip when both scores are same as last', () => {
        const game = makeGame(7, 5, [{s1: 3, s2: 0}, {s1: 7, s2: 5}]);
        updateScoreHistory(game);
        updateScoreHistory(game);
        updateScoreHistory(game);
        expect(game.score_history.length).toBe(2);
    ...
    it('big jumps in score (e.g. carreaux worth 3 points)', () => {
        ...
        const steps = [[3,0],[3,1],[6,1],[6,4],[6,7],[6,13]];
        for (const [a, b] of steps) {
            game.team_1_score = a;
            game.team_2_score = b;
            updateScoreHistory(game);
    ...
    it('both scores increase simultaneously creates new chip', () => {
        ...
        game.team_1_score = 5; game.team_2_score = 4;
        updateScoreHistory(game);
        expect(game.score_history).toEqual([{s1: 3, s2: 2}, {s1: 5, s2: 4}]);
    });

    it('score_history already has entries, continues correctly', () => {
        const game = makeGame(7, 5, [{s1: 2, s2: 0}, {s1: 2, s2: 3}, {s1: 7, s2: 3}, {s1: 7, s2: 5}]);

        game.team_1_score = 9; game.team_2_score = 5;
        updateScoreHistory(game);

        game.team_1_score = 9; game.team_2_score = 8;
        updateScoreHistory(game);
